...
// Spawns a detached worker that attaches to the ART runtime, installs the CodeItem-restore
// choke hook, and dumps each app dex to <app_data_dir>/unpack (or /data/local/tmp/unpack if
// null). `process_name` is the app's nice name (caller passes it; /proc/self/cmdline is still
// "zygote64" at postAppSpecialize time). Safe to call unconditionally.
